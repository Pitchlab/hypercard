...
### Fixed
- **Embeddings were never backfilled, so semantic/hybrid search silently covered only part of the corpus** (#2). Embedding generation was gated solely on `content_hash` change, so cards first indexed while no embedder was available (one-shot CLI, or the daemon before its model finished warm-loading) kept a stored hash and were skipped on every later `index` — they never got an embedding. Now `index` / `index --only` embed a card when its content changed **or** it has no embedding yet (the unchanged-and-already-embedded fast path is preserved). The daemon also auto-backfills missing embeddings as soon as the model finishes loading, and `status` now reports `embeddings_pending` + a warning when `embeddings < cards` instead of failing silently.
...
- **Tests**: +67 new tests (281 total) covering code-block stripping, linker insert/remove safety, the canonical rename + collision guard, the serial queue + startup lock, the temporal layer + search overhaul (timestamp derivation, date-boundary parsing, range filters, `--format` shaping, `--traverse` compact-neighborhood building), pre-timestamp DB migration, and embedding backfill (missing-embedding detection in full + single-card indexing).
